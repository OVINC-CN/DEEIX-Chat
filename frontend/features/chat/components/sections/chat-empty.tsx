...
export function ChatEmptyState({ greetingTitle, badgeLabel, badgeTooltip, contentWidthClassName = "max-w-[1080px]", children }: ChatEmptyStateProps) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col items-center justify-center px-3 py-12 text-center md:px-6 md:py-20">
      <div className={cn("relative w-full", contentWidthClassName)}>
        <motion.div
          layout
          className="absolute inset-x-0 bottom-full mb-7 flex justify-center md:mb-8"
        >
          <AnimatePresence mode="wait" initial={false}>
            <motion.div
              key={titleGroupKey}
              className="relative inline-flex min-w-0 max-w-[calc(100%-4.5rem)] justify-center"
